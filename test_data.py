...
for i in range(maxite):
    pulse = np.random.rand(N)
    M_list[:,i],F_list[:,i],f_story[:,i],times[:,i] = ARS.train(pulse,N,T,alpha,v,L=5,data = np.ones(2), Noise = noise,maxepochs=maxepochs)
    pulse = np.random.rand(N)
...
time_average_A = np.average(times,axis = 1)
f_story_avg_A = np.average(F_list,axis = 1)
f_story_std_A = np.std(F_list,axis = 1)
f_story_max_A = np.max(F_list,axis = 1)
...
ax1.set_title('QSL ARS graph')
fig.show()
np.savez('data_only.npz', pulses = M_list,infidelities = F_list[-1,:])